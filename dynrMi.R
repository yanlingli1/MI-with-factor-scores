...
dynrmi <- function(dynrModel, which.aux=NULL, 
	...
	if(lag < 1){
	  ...
	  if (lag > nt-1){
	    ...
	    # If using this, "lag" can only be a number, instead of a vector
	    datalag <- setDT(dataforlag)[, 
	                                 paste0(rep(which.lag, each = lag), 
	                                        paste0("lag",1:lag)) :=shift(.SD, 1:lag), 
	                                 ID, .SDcols = which.lag]
	    datalag <- data.frame(datalag)
	    datalag <- datalag[,-which(names(datalag) %in% which.lag)]
	  }
	}
	...
	if(lead < 1){
	  ...
	  datalead <- dataforlead
	} else{	
	  if (lead > nt-1){
	    lead <- nt-1
	    warning("Number of leads should be smaller than the number of time points.")
	  }else{
	    datalead <- setDT(dataforlead)[, 
	                                   paste0(rep(which.lead, each = lead), 
	                                          paste0("lead",1:lead)) :=shift(.SD, 1:lead, 
	                                                                         type = 'lead'), 
	                                   ID, .SDcols = which.lead]  
	    datalead <- data.frame(datalead)
	    datalead <- datalead[,-which(names(datalead) %in% which.lead)]
	  }
	}
	...
	diag.mi = function(imp, nvariables,variables,m,itermin=2,iter,burn=0){ 
	  ...
	  chainmean = imp$chainMean[variables,,]
	...
	for(j in 1:m){
		...
		trial <- dynr.cook(modelnew, verbose = verbose, debug_flag=TRUE)  
		if (trial$exitflag < 0 | sum(trial$'bad.standard.errors') > 0){
		  failedtrial[j] = 1
		}
